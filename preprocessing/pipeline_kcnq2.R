...
str_kcnq2 <- data.table()

# TODO
str_kcnq2 <-
  readr::read_delim("features/kcnq2/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:873) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnq2)

str_kcnq2 <-
  readr::read_delim("features/kcnq2/query.isis", delim = " ", skip = 67, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnq2)

str_kcnq2 <-
  readr::read_delim("features/kcnq2/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:872) %>%
  select(X3, X5, X7) %>%
...
str_kcnq2 <-
  readr::read_delim("features/kcnq2/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:873) %>%
...
str_kcnq2 <-
  readr::read_delim("features/kcnq2/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnq2 <-
  readr::read_delim("features/kcnq2/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:872) %>%
...
str_kcnq2$str_top <- NA

str_kcnq2$str_top[1:91] <- "cytoplasmic"
str_kcnq2$str_top[92:112] <- "S1"
str_kcnq2$str_top[113:122] <- "extracellular"
str_kcnq2$str_top[123:143] <- "S2"
str_kcnq2$str_top[144:166] <- "cytoplasmic"
str_kcnq2$str_top[167:187] <- "S3"
str_kcnq2$str_top[188:195] <- "extracellular"
str_kcnq2$str_top[196:218] <- "S4"
str_kcnq2$str_top[219:231] <- "cytoplasmic"
str_kcnq2$str_top[232:252] <- "S5"
str_kcnq2$str_top[253:264] <- "extracellular"
str_kcnq2$str_top[265:285] <- "porehelix"
str_kcnq2$str_top[286:291] <- "extracellular"
str_kcnq2$str_top[292:312] <- "S6"
str_kcnq2$str_top[313:872] <- "cytoplasmic"

str_kcnq2$str_top[is.na(str_kcnq2$str_top)] <- "none"

# features/structure/motif
# method: additional motifs via expert annotation. done manually for now.
str_kcnq2$str_exp <- NA

str_kcnq2$str_exp[277:282] <- "selectivityfilter"
str_kcnq2$str_exp[317:539] <- "calmodulin"
str_kcnq2$str_exp[219:231] <- "S4S5linker"

str_kcnq2$str_exp[198] <- "gating"
str_kcnq2$str_exp[201] <- "gating"
str_kcnq2$str_exp[204] <- "gating"
str_kcnq2$str_exp[207] <- "gating"
str_kcnq2$str_exp[210] <- "gating"
str_kcnq2$str_exp[213] <- "gating"

str_kcnq2$str_exp[is.na(str_kcnq2$str_exp)] <- "none"
...
str_kcnq2 <- readr::read_delim("features/kcnq2/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X3)) %>%
  select(X6) %>%
  rename(str_paraz = X6) %>%
  slice(1:872) %>%
...
str_kcnq2 <-
  readr::read_delim("features/kcnq2/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcnq2$pos <- seq_len(nrow(str_kcnq2))
str_kcnq2$ecdf <- ecdf(str_kcnq2$pos)(str_kcnq2$pos)

# now merge all prepared tables with input
kcnq2 <- read.xlsx(xlsxFile = "input/kcnq2.xlsx", sheet = 1, colNames = TRUE)
...
kcnq2 <- kcnq2 %>%
  ...
  merge(str_kcnq2, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnq2)) > 0){
