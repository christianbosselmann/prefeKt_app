...
str_kcnh2 <- data.table()

str_kcnh2 <-
  readr::read_delim("features/kcnh2/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:1160) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnh2)

str_kcnh2 <-
  readr::read_delim("features/kcnh2/query.isis", delim = " ", skip = 87, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnh2)

str_kcnh2 <-
  readr::read_delim("features/kcnh2/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:1159) %>%
  select(X3, X5, X7) %>%
...
str_kcnh2 <-
  readr::read_delim("features/kcnh2/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:1160) %>%
...
str_kcnh2 <-
  readr::read_delim("features/kcnh2/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnh2 <-
  readr::read_delim("features/kcnh2/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:1159) %>%
...
str_kcnh2$str_top <- NA

str_kcnh2$str_top[1:403] <- "cytoplasmic"
str_kcnh2$str_top[404:424] <- "S1"
str_kcnh2$str_top[425:450] <- "extracellular"
str_kcnh2$str_top[451:471] <- "S2"
str_kcnh2$str_top[472:495] <- "cytoplasmic"
str_kcnh2$str_top[496:516] <- "S3"
str_kcnh2$str_top[517:520] <- "extracellular"
str_kcnh2$str_top[521:541] <- "S4"
str_kcnh2$str_top[542:547] <- "cytoplasmic"
str_kcnh2$str_top[548:568] <- "S5"
str_kcnh2$str_top[569:611] <- "extracellular"
str_kcnh2$str_top[612:632] <- "porehelix"
str_kcnh2$str_top[633:638] <- "extracellular"
str_kcnh2$str_top[639:659] <- "S6"
str_kcnh2$str_top[660:1159] <- "cytoplasmic"

str_kcnh2$str_top[is.na(str_kcnh2$str_top)] <- "none"
...
str_kcnh2$str_exp <- NA

str_kcnh2$str_exp[624:629] <- "selectivityfilter"
str_kcnh2$str_exp[542:547] <- "S4S5linker"

str_kcnh2$str_exp[is.na(str_kcnh2$str_exp)] <- "none"
...
str_kcnh2 <- readr::read_delim("features/kcnh2/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X14)) %>%
  select(X19) %>%
...
str_kcnh2 <-
  readr::read_delim("features/kcnh2/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcnh2$pos <- seq_len(nrow(str_kcnh2))
str_kcnh2$ecdf <- ecdf(str_kcnh2$pos)(str_kcnh2$pos)

# now merge all prepared tables with input
kcnh2 <- read.xlsx(xlsxFile = "input/kcnh2.xlsx", sheet = 1, colNames = TRUE)
...
kcnh2 <- kcnh2 %>%
  ...
  merge(str_kcnh2, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnh2)) > 0){
