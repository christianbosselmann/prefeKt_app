...
str_kcnq5 <- data.table()

str_kcnq5 <-
  readr::read_delim("features/kcnq5/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:933) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnq5)

str_kcnq5 <-
  readr::read_delim("features/kcnq5/query.isis", delim = " ", skip = 72, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnq5)

str_kcnq5 <-
  readr::read_delim("features/kcnq5/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:932) %>%
  select(X3, X5, X7) %>%
...
str_kcnq5 <-
  readr::read_delim("features/kcnq5/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:933) %>%
...
str_kcnq5 <-
  readr::read_delim("features/kcnq5/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnq5 <-
  readr::read_delim("features/kcnq5/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:932) %>%
...
str_kcnq5$str_top <- NA

str_kcnq5$str_top[1:125] <- "cytoplasmic"
str_kcnq5$str_top[126:146] <- "S1"
str_kcnq5$str_top[147:156] <- "extracellular"
str_kcnq5$str_top[157:177] <- "S2"
str_kcnq5$str_top[178:200] <- "cytoplasmic"
str_kcnq5$str_top[201:221] <- "S3"
str_kcnq5$str_top[222:229] <- "extracellular"
str_kcnq5$str_top[230:252] <- "S4"
str_kcnq5$str_top[253:266] <- "cytoplasmic"
str_kcnq5$str_top[267:287] <- "S5"
str_kcnq5$str_top[288:298] <- "extracellular"
str_kcnq5$str_top[299:319] <- "porehelix"
str_kcnq5$str_top[320:325] <- "extracellular"
str_kcnq5$str_top[326:346] <- "S6"
str_kcnq5$str_top[347:932] <- "cytoplasmic"

str_kcnq5$str_top[is.na(str_kcnq5$str_top)] <- "none"

# features/structure/motif
# method: additional motifs via expert annotation. done manually for now.
str_kcnq5$str_exp <- NA

str_kcnq5$str_exp[311:316] <- "selectivityfilter"
str_kcnq5$str_exp[253:266] <- "S4S5linker"
str_kcnq5$str_exp[232] <- "gating"
str_kcnq5$str_exp[235] <- "gating"
str_kcnq5$str_exp[238] <- "gating"
str_kcnq5$str_exp[241] <- "gating"
str_kcnq5$str_exp[244] <- "gating"
str_kcnq5$str_exp[247] <- "gating"

str_kcnq5$str_exp[is.na(str_kcnq5$str_exp)] <- "none"
...
str_kcnq5 <- readr::read_delim("features/kcnq5/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X5)) %>%
  select(X6) %>%
  rename(str_paraz = X6) %>%
  slice(1:932) %>%
...
str_kcnq5 <-
  readr::read_delim("features/kcnq5/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcnq5$pos <- seq_len(nrow(str_kcnq5))
str_kcnq5$ecdf <- ecdf(str_kcnq5$pos)(str_kcnq5$pos)

# now merge all prepared tables with input
kcnq5 <- read.xlsx(xlsxFile = "input/kcnq5.xlsx", sheet = 1, colNames = TRUE)
...
kcnq5 <- kcnq5 %>%
  ...
  merge(str_kcnq5, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnq5)) > 0){
