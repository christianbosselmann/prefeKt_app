...
str_kcna4 <- data.table()

str_kcna4 <-
  readr::read_delim("features/kcna4/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:654) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcna4)

str_kcna4 <-
  readr::read_delim("features/kcna4/query.isis", delim = " ", skip = 51, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcna4)

str_kcna4 <-
  readr::read_delim("features/kcna4/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:653) %>%
  select(X3, X5, X7) %>%
...
str_kcna4 <-
  readr::read_delim("features/kcna4/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:654) %>%
...
str_kcna4 <-
  readr::read_delim("features/kcna4/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcna4 <-
  readr::read_delim("features/kcna4/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:653) %>%
...
str_kcna4$str_top <- NA

str_kcna4$str_top[1:304] <- "cytoplasmic"
str_kcna4$str_top[305:326] <- "S1"
str_kcna4$str_top[327:370] <- "extracellular"
str_kcna4$str_top[371:392] <- "S2"
str_kcna4$str_top[393:403] <- "cytoplasmic"
str_kcna4$str_top[404:424] <- "S3"
str_kcna4$str_top[425:439] <- "extracellular"
str_kcna4$str_top[440:460] <- "S4"
str_kcna4$str_top[461:475] <- "cytoplasmic"
str_kcna4$str_top[476:497] <- "S5"
str_kcna4$str_top[498:511] <- "extracellular"
str_kcna4$str_top[512:531] <- "porehelix"
str_kcna4$str_top[532:538] <- "extracellular"
str_kcna4$str_top[539:567] <- "S6"
str_kcna4$str_top[568:653] <- "cytoplasmic"

str_kcna4$str_top[is.na(str_kcna4$str_top)] <- "none"

# features/structure/motif
# method: additional motifs via expert annotation. done manually for now.
# gating charges and pvp aligned via per viewer
str_kcna4$str_exp <- NA

str_kcna4$str_exp[461:475] <- "S4S5linker"
str_kcna4$str_exp[524:529] <- "selectivityfilter"
str_kcna4$str_exp[651:653] <- "pdzbinding"
str_kcna4$str_exp[555:557] <- "pvp"

str_kcna4$str_exp[444] <- "gating"
str_kcna4$str_exp[447] <- "gating"
str_kcna4$str_exp[450] <- "gating"
str_kcna4$str_exp[453] <- "gating"
str_kcna4$str_exp[456] <- "gating"
str_kcna4$str_exp[459] <- "gating"

str_kcna4$str_exp[is.na(str_kcna4$str_exp)] <- "none"
...
str_kcna4 <- readr::read_delim("features/kcna4/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X7)) %>%
  select(X11) %>%
...
str_kcna4 <-
  readr::read_delim("features/kcna4/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcna4$pos <- seq_len(nrow(str_kcna4))
str_kcna4$ecdf <- ecdf(str_kcna4$pos)(str_kcna4$pos)

# now merge all prepared tables with input
kcna4 <- read.xlsx(xlsxFile = "input/kcna4.xlsx", sheet = 1, colNames = TRUE)
...
kcna4 <- kcna4 %>%
  ...
  merge(str_kcna4, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcna4)) > 0){
