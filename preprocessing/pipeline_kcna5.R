...
str_kcna5 <- data.table()

str_kcna5 <-
  readr::read_delim("features/kcna5/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:614) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcna5)

str_kcna5 <-
  readr::read_delim("features/kcna5/query.isis", delim = " ", skip = 49, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcna5)

str_kcna5 <-
  readr::read_delim("features/kcna5/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:613) %>%
  select(X3, X5, X7) %>%
...
str_kcna5 <-
  readr::read_delim("features/kcna5/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:614) %>%
...
str_kcna5 <-
  readr::read_delim("features/kcna5/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcna5 <-
  readr::read_delim("features/kcna5/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:613) %>%
...
str_kcna5$str_top <- NA

str_kcna5$str_top[1:247] <- "cytoplasmic"
str_kcna5$str_top[248:269] <- "S1"
str_kcna5$str_top[270:323] <- "extracellular"
str_kcna5$str_top[324:345] <- "S2"
str_kcna5$str_top[346:356] <- "cytoplasmic"
str_kcna5$str_top[357:377] <- "S3"
str_kcna5$str_top[378:395] <- "extracellular"
str_kcna5$str_top[396:416] <- "S4"
str_kcna5$str_top[417:431] <- "cytoplasmic"
str_kcna5$str_top[432:453] <- "S5"
str_kcna5$str_top[454:467] <- "extracellular"
str_kcna5$str_top[468:487] <- "porehelix"
str_kcna5$str_top[488:494] <- "extracellular"
str_kcna5$str_top[495:523] <- "S6"
str_kcna5$str_top[524:613] <- "cytoplasmic"

str_kcna5$str_top[is.na(str_kcna5$str_top)] <- "none"
...
str_kcna5$str_exp <- NA

str_kcna5$str_exp[400] <- "gating"
str_kcna5$str_exp[403] <- "gating"
str_kcna5$str_exp[406] <- "gating"
str_kcna5$str_exp[409] <- "gating"
str_kcna5$str_exp[412] <- "gating"
str_kcna5$str_exp[415] <- "gating"
str_kcna5$str_exp[511:513] <- "pvp"

str_kcna5$str_exp[418:431] <- "S4S5linker"
str_kcna5$str_exp[480:485] <- "selectivityfilter"
str_kcna5$str_exp[611:613] <- "pdzbinding"

str_kcna5$str_exp[is.na(str_kcna5$str_exp)] <- "none"
...
str_kcna5 <- readr::read_delim("features/kcna5/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X8)) %>%
  select(X11) %>%
...
str_kcna5 <-
  readr::read_delim("features/kcna5/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcna5$pos <- seq_len(nrow(str_kcna5))
str_kcna5$ecdf <- ecdf(str_kcna5$pos)(str_kcna5$pos)

# now merge all prepared tables with input
kcna5 <- read.xlsx(xlsxFile = "input/kcna5.xlsx", sheet = 1, colNames = TRUE)
...
kcna5 <- kcna5 %>%
  ...
  merge(str_kcna5, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcna5)) > 0){
