...
str_kcnc2 <- data.table()

str_kcnc2 <-
  readr::read_delim("features/kcnc2/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:639) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnc2)

str_kcnc2 <-
  readr::read_delim("features/kcnc2/query.isis", delim = " ", skip = 48, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnc2)

str_kcnc2 <-
  readr::read_delim("features/kcnc2/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:638) %>%
  select(X3, X5, X7) %>%
...
str_kcnc2 <-
  readr::read_delim("features/kcnc2/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:639) %>%
...
str_kcnc2 <-
  readr::read_delim("features/kcnc2/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnc2 <-
  readr::read_delim("features/kcnc2/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:638) %>%
...
str_kcnc2$str_top <- NA

str_kcnc2$str_top[1:229] <- "cytoplasmic"
str_kcnc2$str_top[230:250] <- "S1"
str_kcnc2$str_top[251:283] <- "extracellular"
str_kcnc2$str_top[284:303] <- "S2"
str_kcnc2$str_top[304:313] <- "cytoplasmic"
str_kcnc2$str_top[314:334] <- "S3"
str_kcnc2$str_top[335:345] <- "extracellular"
str_kcnc2$str_top[346:368] <- "S4"
str_kcnc2$str_top[369:381] <- "cytoplasmic"
str_kcnc2$str_top[382:402] <- "S5"
str_kcnc2$str_top[403:452] <- "extracellular"
str_kcnc2$str_top[453:473] <- "S6"
str_kcnc2$str_top[474:638] <- "cytoplasmic"

str_kcnc2$str_top[is.na(str_kcnc2$str_top)] <- "none"
...
str_kcnc2$str_exp <- NA

str_kcnc2$str_exp[437:442] <- "selectivityfilter"
str_kcnc2$str_exp[369:381] <- "S4S5linker"
str_kcnc2$str_exp[348] <- "gating"
str_kcnc2$str_exp[351] <- "gating"
str_kcnc2$str_exp[354] <- "gating"
str_kcnc2$str_exp[357] <- "gating"

str_kcnc2$str_exp[is.na(str_kcnc2$str_exp)] <- "none"
...
str_kcnc2 <- readr::read_delim("features/kcnc2/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X4)) %>%
  select(X6) %>%
  rename(str_paraz = X6) %>%
  add_column(str_kcnc2)
...
str_kcnc2 <-
  readr::read_delim("features/kcnc2/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcnc2$pos <- seq_len(nrow(str_kcnc2))
str_kcnc2$ecdf <- ecdf(str_kcnc2$pos)(str_kcnc2$pos)

# now merge all prepared tables with input
kcnc2 <- read.xlsx(xlsxFile = "input/kcnc2.xlsx", sheet = 1, colNames = TRUE)
...
kcnc2 <- kcnc2 %>%
  ...
  merge(str_kcnc2, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnc2)) > 0){
