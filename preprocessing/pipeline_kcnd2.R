...
str_kcnd2 <- data.table()

str_kcnd2 <-
  readr::read_delim("features/kcnd2/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:631) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnd2)

str_kcnd2 <-
  readr::read_delim("features/kcnd2/query.isis", delim = " ", skip = 48, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnd2)

str_kcnd2 <-
  readr::read_delim("features/kcnd2/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:630) %>%
  select(X3, X5, X7) %>%
...
str_kcnd2 <-
  readr::read_delim("features/kcnd2/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:631) %>%
...
str_kcnd2 <-
  readr::read_delim("features/kcnd2/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnd2 <-
  readr::read_delim("features/kcnd2/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:630) %>%
...
str_kcnd2$str_top <- NA

str_kcnd2$str_top[1:182] <- "cytoplasmic"
str_kcnd2$str_top[183:204] <- "S1"
str_kcnd2$str_top[205:228] <- "extracellular"
str_kcnd2$str_top[229:250] <- "S2"
str_kcnd2$str_top[251:261] <- "cytoplasmic"
str_kcnd2$str_top[262:279] <- "S3"
str_kcnd2$str_top[280:286] <- "extracellular"
str_kcnd2$str_top[287:306] <- "S4"
str_kcnd2$str_top[307:321] <- "cytoplasmic"
str_kcnd2$str_top[322:343] <- "S5"
str_kcnd2$str_top[344:357] <- "extracellular"
str_kcnd2$str_top[358:369] <- "porehelix"
str_kcnd2$str_top[370:384] <- "extracellular"
str_kcnd2$str_top[385:413] <- "S6"
str_kcnd2$str_top[414:630] <- "cytoplasmic"

str_kcnd2$str_top[is.na(str_kcnd2$str_top)] <- "none"

# features/structure/motif
# method: additional motifs via expert annotation. done manually for now.
str_kcnd2$str_exp <- NA

str_kcnd2$str_exp[370:375] <- "selectivityfilter"
str_kcnd2$str_exp[627:630] <- "pdzbinding"
str_kcnd2$str_exp[308:321] <- "S4S5linker"
str_kcnd2$str_exp[401:403] <- "pvp"

str_kcnd2$str_exp[293] <- "gating"
str_kcnd2$str_exp[296] <- "gating"
str_kcnd2$str_exp[299] <- "gating"
str_kcnd2$str_exp[302] <- "gating"
str_kcnd2$str_exp[305] <- "gating"
str_kcnd2$str_exp[308] <- "gating"

str_kcnd2$str_exp[is.na(str_kcnd2$str_exp)] <- "none"
...
str_kcnd2 <- readr::read_delim("features/kcnd2/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X4)) %>%
  select(X6) %>%
  rename(str_paraz = X6) %>%
  add_column(str_kcnd2)
...
str_kcnd2 <-
  readr::read_delim("features/kcnd2/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
  select("rsa", "q3", "q8") %>%
...
str_kcnd2$pos <- seq_len(nrow(str_kcnd2))
str_kcnd2$ecdf <- ecdf(str_kcnd2$pos)(str_kcnd2$pos)

# now merge all prepared tables with input
kcnd2 <- read.xlsx(xlsxFile = "input/kcnd2.xlsx", sheet = 1, colNames = TRUE)
...
kcnd2 <- kcnd2 %>%
  ...
  merge(str_kcnd2, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnd2)) > 0){
