...
str_kcnq1 <- data.table()

str_kcnq1 <-
  readr::read_delim("features/kcnq1/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:677) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnq1)

str_kcnq1 <-
  readr::read_delim("features/kcnq1/query.isis", delim = " ", skip = 51, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnq1)

str_kcnq1 <-
  readr::read_delim("features/kcnq1/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:676) %>%
  select(X3, X5, X7) %>%
...
str_kcnq1 <-
  readr::read_delim("features/kcnq1/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:677) %>%
...
str_kcnq1 <-
  readr::read_delim("features/kcnq1/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnq1 <-
  readr::read_delim("features/kcnq1/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:676) %>%
...
str_kcnq1$str_top <- NA

str_kcnq1$str_top[1:121] <- "cytoplasmic"
str_kcnq1$str_top[122:142] <- "S1"
str_kcnq1$str_top[143:147] <- "extracellular"
str_kcnq1$str_top[148:168] <- "S2"
str_kcnq1$str_top[169:196] <- "cytoplasmic"
str_kcnq1$str_top[197:217] <- "S3"
str_kcnq1$str_top[218:225] <- "extracellular"
str_kcnq1$str_top[226:248] <- "S4"
str_kcnq1$str_top[249:261] <- "cytoplasmic"
str_kcnq1$str_top[262:282] <- "S5"
str_kcnq1$str_top[283:299] <- "extracellular"
str_kcnq1$str_top[300:320] <- "porehelix"
str_kcnq1$str_top[321:327] <- "extracellular"
str_kcnq1$str_top[328:348] <- "S6"
str_kcnq1$str_top[349:676] <- "cytoplasmic"

str_kcnq1$str_top[is.na(str_kcnq1$str_top)] <- "none"
...
str_kcnq1$str_exp <- NA

str_kcnq1$str_exp[312:317] <- "selectivityfilter"
str_kcnq1$str_exp[249:261] <- "S4S5linker"

str_kcnq1$str_exp[228] <- "gating"
str_kcnq1$str_exp[231] <- "gating"
str_kcnq1$str_exp[234] <- "gating"
str_kcnq1$str_exp[237] <- "gating"
str_kcnq1$str_exp[240] <- "gating"
str_kcnq1$str_exp[243] <- "gating"

str_kcnq1$str_exp[is.na(str_kcnq1$str_exp)] <- "none"
...
# problem: no paralog family for KCNQ1, hence no parazscore. use dummy value.
str_kcnq1$str_paraz <- 1
...
str_kcnq1 <-
  readr::read_delim("features/kcnq1/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcnq1$pos <- seq_len(nrow(str_kcnq1))
str_kcnq1$ecdf <- ecdf(str_kcnq1$pos)(str_kcnq1$pos)

# now merge all prepared tables with input
kcnq1 <- read.xlsx(xlsxFile = "input/kcnq1.xlsx", sheet = 1, colNames = TRUE)
...
kcnq1 <- kcnq1 %>%
  ...
  merge(str_kcnq1, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnq1)) > 0){
