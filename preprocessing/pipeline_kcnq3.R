...
str_kcnq3 <- data.table()

str_kcnq3 <-
  readr::read_delim("features/kcnq3/query.consurf.grades", delim = "\t", skip = 12, col_names = TRUE) %>%
  slice(2:873) %>%
  select(SCORE) %>%
  rename(str_consurf = SCORE) %>%
  add_column(str_kcnq3)

str_kcnq3 <-
  readr::read_delim("features/kcnq3/query.isis", delim = " ", skip = 67, col_names = FALSE) %>%
  select(X3) %>%
  rename(str_isis = X3) %>%
  add_column(str_kcnq3)

str_kcnq3 <-
  readr::read_delim("features/kcnq3/query.mdisorder", delim = "\t", skip = 1, col_names = FALSE) %>%
  slice(1:872) %>%
  select(X3, X5, X7) %>%
...
str_kcnq3 <-
  readr::read_delim("features/kcnq3/query.phdRdb", delim = "\t", skip = 42, col_names = TRUE) %>%
  slice(2:873) %>%
...
str_kcnq3 <-
  readr::read_delim("features/kcnq3/query.prof1Rdb", delim = "\t", skip = 78, col_names = TRUE) %>%
...
str_kcnq3 <-
  readr::read_delim("features/kcnq3/iupred.html", delim = "\t", skip = 4, col_names = TRUE) %>%
  slice(1:872) %>%
...
str_kcnq3$str_top <- NA

str_kcnq3$str_top[1:121] <- "cytoplasmic"
str_kcnq3$str_top[122:142] <- "S1"
str_kcnq3$str_top[143:152] <- "extracellular"
str_kcnq3$str_top[153:173] <- "S2"
str_kcnq3$str_top[174:196] <- "cytoplasmic"
str_kcnq3$str_top[197:217] <- "S3"
str_kcnq3$str_top[218:225] <- "extracellular"
str_kcnq3$str_top[226:247] <- "S4"
str_kcnq3$str_top[248:261] <- "cytoplasmic"
str_kcnq3$str_top[262:282] <- "S5"
str_kcnq3$str_top[283:303] <- "extracellular"
str_kcnq3$str_top[304:324] <- "porehelix"
str_kcnq3$str_top[325:330] <- "extracellular"
str_kcnq3$str_top[331:351] <- "S6"
str_kcnq3$str_top[352:872] <- "cytoplasmic"

str_kcnq3$str_top[is.na(str_kcnq3$str_top)] <- "none"

# features/structure/motif
# method: additional motifs via expert annotation. done manually for now.
str_kcnq3$str_exp <- NA

str_kcnq3$str_exp[316:321] <- "selectivityfilter"
str_kcnq3$str_exp[356:537] <- "calmodulin"
str_kcnq3$str_exp[248:261] <- "S4S5linker"

str_kcnq3$str_exp[227] <- "gating"
str_kcnq3$str_exp[230] <- "gating"
str_kcnq3$str_exp[233] <- "gating"
str_kcnq3$str_exp[236] <- "gating"
str_kcnq3$str_exp[239] <- "gating"
str_kcnq3$str_exp[242] <- "gating"

str_kcnq3$str_exp[is.na(str_kcnq3$str_exp)] <- "none"
...
str_kcnq3 <- readr::read_delim("features/kcnq3/query.per.txt", delim = "\t", skip = 1, col_names = FALSE) %>%
  filter(!grepl('-', X3)) %>%
  select(X6) %>%
  rename(str_paraz = X6) %>%
  slice(1:872) %>%
...
str_kcnq3 <-
  readr::read_delim("features/kcnq3/netsurfp.csv", delim = ",", skip = 0, col_names = TRUE) %>%
...
str_kcnq3$pos <- seq_len(nrow(str_kcnq3))
str_kcnq3$ecdf <- ecdf(str_kcnq3$pos)(str_kcnq3$pos)

# now merge all prepared tables with input
kcnq3 <- read.xlsx(xlsxFile = "input/kcnq3.xlsx", sheet = 1, colNames = TRUE)
...
kcnq3 <- kcnq3 %>%
  ...
  merge(str_kcnq3, by.x = "pos", by.y = "pos", all = FALSE)

# sanity check
if (sum(is.na(kcnq3)) > 0){
